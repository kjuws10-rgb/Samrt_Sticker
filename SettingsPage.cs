...
public sealed class SettingsPage : Grid
{
    public SettingsPage(SettingsStore store, Action goBack)
    {
        ...
        panel.Children.Add(new TextBlock { Text = "모든 메모 펼치기 / 닫기 단축키", Margin = new Thickness(0, 14, 0, 3) }); var showAllHotkey = new TextBox { Text = s.ShowAllNotesShortcut, Padding = new Thickness(8, 5, 8, 5), ToolTip = "한 번 누르면 펼치고, 다시 누르면 모두 숨깁니다." }; panel.Children.Add(showAllHotkey);
        ...
        save.Click += (_, _) => { if (!HotkeyManager.TryParse(hotkey.Text, out _, out _)) { System.Windows.MessageBox.Show("화면 캡처 단축키 형식을 확인하세요. 예: Shift + F5", "Smart Sticker"); return; } if (!HotkeyManager.TryParse(showAllHotkey.Text, out _, out _)) { System.Windows.MessageBox.Show("모든 메모 펼치기 단축키 형식을 확인하세요. 예: Ctrl + Shift + F6", "Smart Sticker"); return; } if (HotkeyManager.AreEquivalent(hotkey.Text, showAllHotkey.Text)) { System.Windows.MessageBox.Show("화면 캡처와 모든 메모 펼치기에 서로 다른 단축키를 지정하세요.", "Smart Sticker"); return; } store.Save(new AppSettings { StartWithWindows = auto.IsChecked == true, RestoreNotesOnLaunch = restore.IsChecked == true, DefaultPinned = pin.IsChecked == true, CopyCaptureToClipboard = copy.IsChecked == true, CaptureShortcut = hotkey.Text.Trim(), ShowAllNotesShortcut = showAllHotkey.Text.Trim(), Theme = theme.SelectedItem?.ToString() ?? "Light", CaptureMode = s.CaptureMode, DefaultFontFamily = s.DefaultFontFamily, DefaultFontSize = s.DefaultFontSize }); goBack(); }; Grid.SetRow(save, 2); Children.Add(save);
    }
